...
const prepareCartResponse= (cart: Cart, total: number) => {
    const cartResponse: CartResponse = {
        data: {
            cart: {
                id: cart.id,
                items: cart.items.getItems()
            },
            total: total
        }
    }
...
cartRouter.put('/', async (req: Request, res: Response<CartResponse>)=> {
    ...
    try {
        ...
        const updatedCart = await cartService.updateUserCart(userId, productToUpdate, count);
